...
function Cart() {
    ...
    const { getProdectData, cartIds, ProductData, setCartIds, deletCart, Productloading, setProductData, ProductError, setProductloading } = useContext(cartcontext)
    const { offersList } = useContext(supbasecontext)

    // Removed redundant local state to fix infinite loop
    ...
    // whenever the applied coupon changes (including on mount) mirror it into the input
    useEffect(() => {
    ...
    const handleApplyCoupon = () => {
        ...
        // console.log('🔍 Coupon entered:', JSON.stringify(couponCode.trim()));
        // console.log('🔍 All coupons in offersList:', offersList?.filter(o => o.offer_type === 'coupon').map(o => ({ code: o.coupon_code, status: o.status, type: o.offer_type })));

        const foundCoupon = offersList?.find(o =>
            o.status === 'نشط' &&
            o.offer_type === 'coupon' &&
        ...
        if (foundCoupon) {
            setAppliedCouponCode(foundCoupon.coupon_code);
            localStorage.setItem('appliedCouponCode', foundCoupon.coupon_code);
            Swal.fire('نجاح', `تم تطبيق الكوبون (${foundCoupon.discount_value}%) بنجاح!`, 'success');
        } else {
            Swal.fire('خطأ', 'كود الكوبون غير صالح أو غير متاح لهذه المشتريات', 'error');
        }
    };

    // Dynamic features from offersList
    const iconsArray = ['📘', '💸', '🎯', '🎁', '🥇', '🌟', '💥'];
    const features = offersList?.filter(o => o.status === 'نشط' && o.offer_type === 'discount' && o.hidden !== true).map((offer, index) => ({
        discount: `${offer.discount_value}%`,
        condition: offer.description,
        icon: iconsArray[index % iconsArray.length],
        amount: offer.min_purchase > 0 ? offer.min_purchase : '',
        egp: 'ج',
        more: offer.min_purchase ? 'كحد أدنى' : '',
        id: offer.id,
        rawOffer: offer
    })) || [];
    ...
    const currentDiscountPercentage = `${discountData.percentage}%`;

    const totalPriceAfterDicount = () => {
        return calculateTotalPriceAfterDiscount(totalPrice, discountData.amount, false);
    }
    ...
    // if the cart becomes empty we should remove any applied coupon as well
    useEffect(() => {
        if (ProductData && ProductData.length === 0 && appliedCouponCode) {
            setAppliedCouponCode('');
            setCouponCode('');
            localStorage.removeItem('appliedCouponCode');
        }
    ...
    const handleCheckout = async (e) => {
        ...
        try {
            ...
            const outOfStockItems = data.filter(item => item.stoke <= 0)
            ...
            if (outOfStockItems.length > 0) {
                ...
                Swal.fire({
                    title: 'تم إزالة بعض الكتب',
                    text: `تم إزالة (${removedNames}) لأن الكمية نفدت من المخزون`,
                    icon: 'warning',
                    confirmButtonText: 'متابعه تاكيد الكتب المتوفره '
                }).then((result) => {
